  ...
  <div class="col-md-12 col-xl-12 center-wrapper">
    <div class="card rounded">
      <div class="card-body">
        ...
        <form method="POST" action="{{ route('update.register') }}" class="forms-sample" enctype="multipart/form-data" class="forms-sample">
        @csrf
        ...
        <input type="hidden" name="id" value="{{ $item->id }}">
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" name="resi_pengiriman" class="form-control @error('resi_pengiriman') is-invalid @enderror" value="{{$item->resi_pengiriman}}">
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" name="owner" class="form-control @error('owner') is-invalid @enderror" value="{{$item->owner}}">
              @error('owner')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="date" name="tanggal_masuk" class="form-control @error('tanggal_masuk') is-invalid @enderror" value="{{$item->tanggal_masuk}}">
              @error('tanggal_masuk')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input type="text" name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" value="{{$item->deskripsi}}" rows="5"></input>
              @error('deskripsi')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
          ...
          <div class="row mt-3 mb-3">
            ...
            <img  id="showImage" class="wd-150" src="{{ (!empty ($item->photo_barang)) ? url('barang_image/' .$item->photo_barang) :url('no_image.jpg')}}" alt="barang">
          </div>
